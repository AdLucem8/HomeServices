(() => {
    ...
    async function loadBlogItems() {
        ...
        if (response.ok) {
            const responseRusult = await response.json();
            data = responseRusult;
    ...
    function buildItem(item) {
        let itemBlock = ``;
        itemBlock += `<article class="blog__item item-blog">`;
        item.image ? itemBlock += `<a href="${item.id}" class="item-blog__image">\n            <img class="item-blog__img ibg" src="${item.image}" alt="Image">\n        </a>` : null;
        itemBlock += `<div class="item-blog__date">${item.date}</div>`;
        itemBlock += `<h4 class="item-blog__title">\n            <a href="${item.url}" class="item-blog__link-title">${item.title}</a>\t\t\t\t\t\t\t\t\t\n        </h4>`;
        item.text ? itemBlock += `<div class="item-blog__text">\n            <p>${item.text}</p>\n        </div>` : null;
        if (item.tags) {
            itemBlock += `<div class="item-blog__tags">`;
            for (const tag in item.tags) itemBlock += `<a href="${item.tags[tag]}" class="item-blog__tag">${tag}</a>`;
            itemBlock += `</div>`;
        }
        itemBlock += `</article>`;
        blogItems.insertAdjacentHTML("beforeend", itemBlock);
    }
    document.addEventListener("click", documentActions);
    function viewMore() {
        const dataItemsLength = data.items.length;
        const currentItems = document.querySelectorAll(".item-blog").length;
        const more = document.querySelector(".blog__view-more");
        currentItems < dataItemsLength ? more.hidden = false : more.hidden = true;
    }
